 ...
 * 
 */
public class GreencampusViewActivity extends FragmentActivity {
	...
	private GreenCampusViewHandler greenCampusViewHandler;

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		// TODO Auto-generated method stub
		super.onCreate(savedInstanceState);
		setContentView(R.layout.greencampus_view_layout);
	...
	public void handleMessage(Message msg) {
		...
		case FILE_DOWNLOAD_FAIL:
			Toast.makeText(getBaseContext(),
					getResources().getString(R.string.msg_download_fail),
					Toast.LENGTH_SHORT).show();
			// set progress state
			progressState = 0;
			break;
	...
	private void init() {
		...
		greenCampusViewHandler = new GreenCampusViewHandler(
				GreencampusViewActivity.this);

		Intent intent = getIntent();
		postingUrl = intent.getExtras().getString("url");
		if (!postingUrl.startsWith("http://")) {
			postingUrl = getString(R.string.complaint_url2) + postingUrl;
		}
	}
	...
	private void showPosting() {
		Thread thread = new Thread(new Runnable() {
			...
			public void run() {
				...
				httpManager.init();
				// set httpPost
				httpManager.setHttpPost(postingUrl);
				// httpResponse
				...
				try {
					...
					if (status.getStatusCode() == HttpStatus.SC_OK) {
						...
						Message msg = greenCampusViewHandler.obtainMessage();
						msg.what = LOADING_SUCCESS;
						msg.obj = elementMap;
						greenCampusViewHandler.sendMessage(msg);
					} else
						greenCampusViewHandler.sendEmptyMessage(LOADING_FAIL);
	...
	private void parsingData(List<Element> elementsList) {
		...
		WebView userWebView = (WebView) findViewById(R.id.green_view_new_user_webview);
		userWebView.getSettings().setLayoutAlgorithm(
				LayoutAlgorithm.SINGLE_COLUMN);
		// get admin content WebView
		WebView adminWebView = (WebView) findViewById(R.id.green_view_new_admin_webview);
		adminWebView.getSettings().setLayoutAlgorithm(
				LayoutAlgorithm.SINGLE_COLUMN);
		/********************
		...
		if (boardViewDiv != null) {
			// get tbodyList
			List<Element> tbodyList = boardViewDiv
					.getAllElements(HTMLElementName.TBODY);
			// set divier visible
			if (tbodyList.size() > 1) {
				((TextView) findViewById(R.id.green_view_new_divider))
						.setVisibility(View.VISIBLE);
				((TextView) findViewById(R.id.green_view_new_admin_divider))
						.setVisibility(View.VISIBLE);
				adminWebView.setVisibility(View.VISIBLE);
			}
			for (int tbodyIndex = 0; tbodyIndex < tbodyList.size(); tbodyIndex++) {
				List<Element> trList = tbodyList.get(tbodyIndex)
				...
				for (int trIndex = 0; trIndex < trList.size(); trIndex++) {
					...
					if (trIndex == 0 && tbodyIndex == 0) {
						// strong tag
						List<Element> strongList = trList.get(trIndex)
						...
						if (strongList.size() > 0) {
							...
							subject = subject.replaceAll("<{1}.[^<>]*>{1}", "")
									.trim();
							// set text
							((TextView) findViewById(R.id.green_view_new_user_subject))
									.setText(subject.trim());
						}
					}
					// 사용자 글 작성자, 일자
					else if (trIndex == 1 && tbodyIndex == 0) {
						// td tag
						List<Element> tdList = trList.get(trIndex)
						...
						if (tdList.size() > 0) {
							...
							((TextView) findViewById(R.id.green_view_new_user_writer))
									.setText(writer.trim());
							((TextView) findViewById(R.id.green_view_new_user_date))
									.setText(date.trim());
						}
					}
					// 사용자, 관리자 글 내용
					...
					// 사용자 글, 관리자 글 첨부 파일
						...
						Element tdElement = trList.get(trIndex)
								.getFirstElement(HTMLElementName.TD);
						// get <a> tag list
						List<Element> aList = tdElement
								.getAllElements(HTMLElementName.A);
						if (aList.size() > 0) {
							for (Element aElement : aList) {
								// get downloadUrl
								String downloadUrl = aElement
										.getAttributeValue("onclick").trim();
								downloadUrl = downloadUrl.substring(downloadUrl
										.indexOf("'") + 1);
								downloadUrl = downloadUrl.substring(0,
										downloadUrl.indexOf("'"));
								if (!downloadUrl.startsWith("http"))
									downloadUrl = "http://www.mju.ac.kr"
											+ downloadUrl;
								// get filename
								String fileName = aElement.getContent()
										.toString();
								if (fileName != null) {
									if (fileName.trim().contains("<img")) {
										...
										addAttachedFileTextView(
												fileName.trim(), downloadUrl,
												tbodyIndex);
									}
								}
							}
					...
					// 관리자 답변 제목
						...
						if (strongList.size() > 0) {
							...
							subject = subject.replaceAll("<{1}.[^<>]*>{1}", "")
									.trim();
							// set text
							((TextView) findViewById(R.id.green_view_new_admin_subject))
									.setVisibility(View.VISIBLE);
							((TextView) findViewById(R.id.green_view_new_admin_subject))
									.setText(subject);
						}
					...
					// 관리자 답변 작성자, 일자
						...
						if (tdList.size() > 0) {
							...
							tdElement = tdList.get(1);
							String date = tdElement.getContent().toString();
							((TextView) findViewById(R.id.green_view_new_admin_writer))
									.setVisibility(View.VISIBLE);
							((TextView) findViewById(R.id.green_view_new_admin_writer))
									.setText(writer);
							((TextView) findViewById(R.id.green_view_new_admin_date))
									.setVisibility(View.VISIBLE);
							((TextView) findViewById(R.id.green_view_new_admin_date))
									.setText(date);
						}
	 ...
	 * @param downloadUrl
	 */
	private void addAttachedFileTextView(String fileName,
			final String downloadUrl, int tbodyIndex) {
		if (fileName != null && !fileName.equals("")) {
			// get complaintContentView layout
			LinearLayout contentView = (LinearLayout) findViewById(R.id.green_view_new_content);
			// get contentLayout
			LinearLayout ll = null;
			if (tbodyIndex == 0)
				ll = (LinearLayout) contentView
						.findViewById(R.id.green_view_new_user_attached_file);
			else
				ll = (LinearLayout) contentView
						.findViewById(R.id.green_view_new_admin_attached_file);
			/***
			 * set attached file textView
			 ***/
			LinearLayout attachll = (LinearLayout) LayoutInflater.from(this)
					.inflate(R.layout.attach_file_layout, null);
			TextView attachFileTextView = (TextView) attachll
					.findViewById(R.id.attach_file_textView);
			...
			attachFileTextView.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					downloadPDialog = new ProgressDialog(
							GreencampusViewActivity.this);
					downloadPDialog
							.setProgressStyle(ProgressDialog.STYLE_HORIZONTAL);
					downloadPDialog.setTitle("");
					downloadPDialog.setCancelable(false);
					downloadPDialog.setMessage(getResources().getString(
							R.string.msg_download_progress));
					downloadPDialog.show();
					// file download
					downloadAttachedFile(((TextView) v).getText().toString(),
							downloadUrl);
				}
			});
			ll.addView(attachll);
	 ...
	 * @param downloadUrl
	 */
	private void downloadAttachedFile(final String fileName,
				...
				if (Environment.getExternalStorageState().equals(
					...
					if (!saveFile.exists()) {
						...
						try {
							...
							greenCampusViewHandler.post(new Runnable() {
								@Override
								public void run() {
									// TODO Auto-generated method stub
							...
							while ((nRead = is.read(filebytes)) > 0) {
								...
								Message msg = greenCampusViewHandler
										.obtainMessage();
					...
					// send message to handler about download fail
						...
						greenCampusViewHandler
								.sendEmptyMessage(FILE_DOWNLOAD_FAIL);
				}
				// does not mounted extra storage
				// so, download fail
				// send message to handler about download fail
				else
					greenCampusViewHandler.sendEmptyMessage(FILE_DOWNLOAD_FAIL);
	...
	private void showDownloadFile(String fileName) {
		...
		try {
			...
			Toast.makeText(
					getBaseContext(),
					getResources().getString(
							R.string.msg_implement_fail_download_file),
					Toast.LENGTH_SHORT).show();
		}
	}
	...
	static class GreenCampusViewHandler extends Handler {
		private final WeakReference<GreencampusViewActivity> greenCampusViewAcivity;

		public GreenCampusViewHandler(GreencampusViewActivity activity) {
			// TODO Auto-generated constructor stub
			greenCampusViewAcivity = new WeakReference<GreencampusViewActivity>(
					activity);
		}
		...
		public void handleMessage(Message msg) {
			// TODO Auto-generated method stub
			GreencampusViewActivity activity = greenCampusViewAcivity.get();
			if (activity != null) {
