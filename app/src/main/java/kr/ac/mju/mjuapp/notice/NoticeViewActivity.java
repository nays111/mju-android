...
public class NoticeViewActivity extends FragmentActivity {
	...
	private NoticeViewHandler noticeViewHandler;
	...
	private void init() {
		...
		if (!postingUrl.startsWith("http://"))
			postingUrl = getString(R.string.community_board_url) + postingUrl;
		String title = intent.getExtras().getString("subtitle");

		((TextView) findViewById(R.id.notice_view_title)).setText("");
		((TextView) findViewById(R.id.notice_view_title)).setText(title);

		fragmentManager = getSupportFragmentManager();
		progressDialog = new MJUProgressDialog();
		noticeViewHandler = new NoticeViewHandler(NoticeViewActivity.this);
	}
			...
			public void run() {
				...
				try {
					...
					if (status.getStatusCode() == HttpStatus.SC_OK) {
						...
						Message msg = noticeViewHandler.obtainMessage();
						msg.what = LOADING_SUCCESS;
						msg.obj = elementMap;
						noticeViewHandler.sendMessage(msg);
					} else
						noticeViewHandler.sendEmptyMessage(LOADING_FAIL);
	...
	private void parsingData(List<Element> elementsList) {
		// get webView
		WebView webView = (WebView) findViewById(R.id.notice_view_new_webview);
		webView.getSettings()
				.setLayoutAlgorithm(LayoutAlgorithm.NARROW_COLUMNS);// SINGLE_COLUMN);
		/********************
		...
		if (boardViewDiv != null) {
			Element tbodyElement = boardViewDiv
					.getFirstElement(HTMLElementName.TBODY);
			if (tbodyElement != null) {
				List<Element> trList = tbodyElement
				...
				for (int trIndex = 0; trIndex < trList.size(); trIndex++) {
					...
					if (trIndex == 0) {
						...
						if (strongList.size() > 0) {
							...
							subject = subject.substring(0,
									subject.indexOf("<div")).trim();// replaceAll("<{1}.[^<>]*>{1}",
																	// "").trim();
							// set text
							((TextView) findViewById(R.id.notice_view_new_subject))
					...
					// 글쓴이, 날짜
						...
						if (tdList.size() > 0) {
							...
							((TextView) findViewById(R.id.notice_view_new_writer))
									.setText(writer.trim());
							((TextView) findViewById(R.id.notice_view_new_date))
									.setText(date.trim());
						}
					}
					// 글 내용
					...
					// 첨부파일
						...
						if (aList.size() > 0) {
							for (Element aElement : aList) {
								...
								// 게시글의 첨부파일 표시하는 HTML 변경된 것 같음
								// 2014.04.11 주석처리
								// String downloadUrl =
								// aElement.getAttributeValue("onclick").trim();
								// downloadUrl =
								// downloadUrl.substring(downloadUrl.indexOf("'")
								// + 1);
								...
								if (!downloadUrl.startsWith("http"))
									downloadUrl = getString(R.string.mju_home_page_url)
											+ downloadUrl;
								// get filename
	...
	private void addAttachedFileTextView(String fileName,
		...
		if (fileName != null && !fileName.equals("")) {
			// get attachfile Layout
			LinearLayout ll = (LinearLayout) findViewById(R.id.notice_view_new_attached_file);
			/***
				...
				if (Environment.getExternalStorageState().equals(
					...
					if (!saveFile.exists()) {
						...
						try {
							...
							noticeViewHandler.post(new Runnable() {
								@Override
								public void run() {
									// TODO Auto-generated method stub
					...
					// send message to handler about download fail
						...
						noticeViewHandler.sendEmptyMessage(FILE_DOWNLOAD_FAIL);
				}
				// do not mounted extra storage
				// so, download fail
				// send message to handler about download fail
				else
					noticeViewHandler.sendEmptyMessage(FILE_DOWNLOAD_FAIL);
	...
	static class NoticeViewHandler extends Handler {
		private final WeakReference<NoticeViewActivity> noticeViewAcivity;

		public NoticeViewHandler(NoticeViewActivity activity) {
			// TODO Auto-generated constructor stub
			noticeViewAcivity = new WeakReference<NoticeViewActivity>(activity);
		}

		@Override
		public void handleMessage(Message msg) {
			// TODO Auto-generated method stub
			NoticeViewActivity activity = noticeViewAcivity.get();
			if (activity != null) {
